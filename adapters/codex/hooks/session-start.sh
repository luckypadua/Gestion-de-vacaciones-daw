#!/usr/bin/env bash
# DAW — codex · session start: materialize the state and boot the pipeline.
set -uo pipefail
...
PLUGIN_ROOT="${DAW_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}"
DAW="$REPO/.daw"
...
exec python3 "$BOOT" --repo "$REPO" --session-id "$SID" --format text --method "$DAW"
