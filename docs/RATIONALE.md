# Why DAW decides what it decides
...
It is here so you can read it **before** you adopt DAW, and disagree on the merits.
...
**Why.** DAST needs a running deployment, and DAW cannot know whether you have one, where it is, or
...
what DAW checks. It says so rather than implying coverage it does not have.
...
**Why.** This one comes from a defect in DAW's own acceptance run. The rules were all there: the PRD
...
requirement. DAW cannot merge for you, so it does the part it can: never start you from a stale base,
...
**The decision.** PRD, spec, threat model, SAST report, verification report — all under `docs/daw/`,
...
**The cost.** Your repository gains a `docs/daw/` directory that grows with every ticket. Reviewers
...
**The decision.** `daw-context-check` reports only knowledge the agent **cannot discover on its own**
— the commands DAW is about to run: pre-commit, the CI invocations, the real test and lint commands.
...
**The cost.** DAW will not warn you about a convention it cannot see evidence for. It reports that
...
**Why it reports rather than refuses.** DAW cannot tell the agent's shell from yours. Editing your
...
and a non-zero exit is logged and skipped. DAW was sending it a refusal it
ignores — so a `.daw-state.json` forged with `sed` was detected correctly, by a
...
**The decision.** When DAW is used to compare tools, each tool rebuilds the same application in its
...
**The decision.** When DAW finds a `.daw-state.json` that cannot be true, the agent is told to stop
...
**Why.** For most of DAW's life the refusal said the opposite — *"fix the state and redo the
...
`rm -f .daw-state.json`, which took the run's history with it. The net had caught the corruption
...
`.daw-journal.jsonl`, written by the hook and never by the model. If the state disappears, or comes
...
This is not DAW's invention and the vocabulary is not ours. Software supply chain calls the artifact
...
Read against that ladder, DAW's gates were all level 1 and one of them is now level 2. Saying so is
the point: a framework whose gates all look alike, while one is evidence and six are records, is
teaching its own users something false about their pipeline.
...
`sast` has nothing to run. DAW's SAST is a reading of the code by a model, plus the package
...
`tests` is the one people will ask for, and the one that would make DAW something it must not
become. Verifying it means DAW runs your suite: knowing whether this is pytest or jest or a
monorepo with five runners, in what directory, with which environment, and being right about it in
somebody else's repository. CI can do that because CI **is** the environment. DAW lives inside your
...
**The cost.** Six of the eight gates rest on the model's report, and this document is the only place
that says so plainly. A reader who skims the word "gate" will assume more than is there, and the
README now spends a paragraph correcting that assumption instead of enjoying it.

Grading also invites the question of when the rest get raised, and the honest answer is: when
something outside the model can answer for them. `spec`, `threat` and `verify` can follow `define`
the day their validators write receipts — the mechanism is one table in `validate-transition.py`,
not new machinery. `tests`, `sast` and `pr` are not on that path, and adding them would cost more
than it is worth.
